...
app.post('/insert', async (req, res) => {
    ...
    const student = new StudentModel({ Name: name, Register_number: rollnumber });
    try {
        await student.save();
...
app.get('/download', (req, res) => {
  ...
  uniqueDates.forEach(date => {
    ...
    if (attendanceRecord && attendanceRecord.attendanceRecords) {
      const attendance = attendanceRecord.attendanceRecords.find(r => r.studentId && r.studentId.Name === name);
      rowData[date] = attendance ? attendance.attendance : '';
    } else {
      rowData[date] = '';
    }
